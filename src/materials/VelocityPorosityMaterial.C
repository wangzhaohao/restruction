...
VelocityPorosityMaterial::validParams()
{
  ...
  params.addClassDescription("the restructing of MOX porosity velocity");
...
VelocityPorosityMaterial::VelocityPorosityMaterial(const InputParameters & parameters)
  ...
  _velocity_porosity(declareProperty<Real>("velocity_porosity"))
{
}
...
// 此处1e7是为了单位一制？大概是吧，不然这个扩散速率太小了
		_velocity_porosity[qp] = 5e-16 * (0.988 + 6.395e-6 * qp_temperature + 3.543e-9 * std::pow(qp_temperature, 2) + 3e-12 * std::pow(qp_temperature, 3)) * std::pow(qp_temperature, -2.5) * delta_H * 1e7 * P0 * std::exp(-delta_H / 8.314 / qp_temperature) * _grad_temperature[qp](0); // 这里使用_qp的原因未知，猜测大概是因为qp设置成unsigned会丢失信息？

		_velocity_porosity[qp] = 0.01* 0.01 * std::abs(_velocity_porosity[qp]);
